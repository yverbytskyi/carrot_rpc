...
module CarrotRpc::CLI
  ...
  def self.add_process_options(option_parser)
    ...
    option_parser.on(" ", "--server_test_mode", "runs servers with '_test' appended to queue names. Set Rails Rack env vars to 'test' when used in conjunction with '--autoload_rails'") do
